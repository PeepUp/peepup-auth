...
interface LocalePhonesRegex {
    [locale: string]: string | RegExp | undefined;
}

/* eslint-disable no-useless-escape */
/* eslint-disable max-len */
const phones: LocalePhonesRegex = {
    ...
    "fr-CA": /^(\+?1)?[2-9]\d{2}[2-9](?!11)\d{6}$/,
    "fr-BE": /^(\+?32|0)4?\d{8}$/,
    "zh-HK": /^(\+?852\-?)?[569]\d{3}\-?\d{4}$/,
    "zh-MO": /^(\+?853\-?)?[6]\d{3}\-?\d{4}$/,
...
export function isMobilePhone(
    ...
    CommonUtil.assertString(str);

    if (!str) {
        throw new Error("Invalid argument supplied for function isMobilePhone");
    }
    ...
    if (Array.isArray(locale)) {
        return locale.some((key) => {
            ...
            if (phones?.[key]) {
                const phone = phones[key] as RegExp;
                if (phone.test(str)) {
                    return true;
                }
            }
    ...
    if ((locale as string) in phones) {
        if (phones[locale as string]) {
            const phone = phones[locale as string] as RegExp;
